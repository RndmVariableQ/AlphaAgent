...
qlib.init(provider_uri="~/.qlib/qlib_data/latest_data")
# qlib.init(provider_uri="~/.qlib/qlib_data/us_data")
from qlib.data import D

instruments = D.instruments()
fields = ["$open", "$close", "$high", "$low", "$volume"]  # , "$amount", "$turn", "$pettm", "$pbmrq"
data = D.features(instruments, fields, freq="day").swaplevel().sort_index().loc["2019-01-01":].sort_index()
...
data = (
    (
        D.features(instruments, fields, start_time="2024-01-01", end_time="2025-02-28", freq="day")
        .swaplevel()
        .sort_index()
    )
    .swaplevel()
